...
// Create a new post, get user_id from cookie
app.post('/create', async (req, res) => {
    const { title, content, channel } = req.body;

    // Assuming user_id is stored in a cookie named 'user_id'
    const user_id = req.cookies.user;
    if (!user_id) {
    return res.status(401).send('Unauthorized access. Please log in to create a post.');
    }
    ...
    await newPost.save();
    res.redirect('/blogs'); // Redirect to the list of blogs after successful creation
    } catch (err) {
...
app.get('/blogs', async (req, res) => {
    ...
    res.status(500).send('Internal server error while fetching blogs.');
    }
});

// Render the create comment page, using blog_id from URL parameter
app.get('/createcomment/:blog_id', async (req, res) => {
    const { blog_id } = req.params;
    const userId = req.cookies.user; // Assuming user_id is still stored in a cookie or session

    // Check if user_id is present
    if (!userId) {
    return res.status(400).send('Missing user ID. Please make sure you are logged in.');
    }

    try {
    // Fetch the blog post details using blog_id from the URL
    const blog = await Post.findOne({ _id: blog_id });
    if (!blog) {
        return res.status(404).send('Blog post not found.');
    }

    // Render the form with the blog title, blog_id, and userId
    res.render('createcomment', { blogTitle: blog.title, blogId: blog_id, userId });
    } catch (err) {
    console.error('Error fetching blog details:', err);
    res.status(500).send('Internal server error while loading comment page.');
    }
});
...
app.post('/comment/create', async (req, res) => {
    ...
    return res.status(400).send('All fields (blog_id, user_id, content) are required.');
    }
...
// Render the blog post and set cookies
app.get('/viewblog/:blog_id', async (req, res) => {
    ...
    // Set cookies for user_id (assuming the user is logged in)
    // Assuming the user_id is stored in session after login
    const userId = req.cookies.user;
    if (!userId) {
        return res.status(401).send('Unauthorized access. Please log in to view this blog post.');
    }

    res.cookie('user_id', userId, { httpOnly: true, maxAge: 24 * 60 * 60 * 1000 }); // Set user_id cookie for 1 day

    // Render the blog post details
    res.render('viewblog', { blog });
    } catch (err) {
    console.error('Error fetching blog details:', err);
    res.status(500).send('Internal server error while loading blog post.');
    }
});
...
app.put('/blog/:id', async (req, res) => {
    ...
    const userId = req.cookies.user; // Retrieve user ID from cookie
    ...
    try {
        ...
        res.status(403).send('Unable to edit - you are not the post owner!');
        }
    } catch (err) {
...
app.put('/comment/:id', async (req, res) => {
    ...
    if (comment && comment.user_id === userId) {
        ...
        // Redirect to the associated blog post to view the updated comment
        res.redirect(`/viewblog/${comment.blog_id}`);
    } else {
        res.status(403).send('Unable to edit - you are not the comment owner!');
    }
    } catch (err) {
